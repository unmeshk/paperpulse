...
10. **Verify one-fetch-per-category.** Confirm the pipeline downloads each RSS feed and generates each category blurb exactly once per day, no matter how many users selected that category (e.g. two users picking cs.LG must not mean two downloads). The design intends this — `get_fetch_list` unions DISTINCT user categories, and blurbs are per-category files shared by all subscribers — but verify with a test that the fetch list is deduplicated and each slug is fetched/summarized once per run.
